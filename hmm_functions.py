...
def Emission_probs_poisson(emissions, observations, weights, mutrates):
    ...
    # genome_mean = 0.0007280896773141077
    # genome_mean *= 2000
    # # avg number of kmers per window of HG01891_h1_vidija_counts.bed
    # mut = np.random.poisson(genome_mean, n)
    # chunk_size = 25
    # for i in range(0, len(mut), chunk_size):
    #     chunk_val = mut[i]
    #     mut[i:i+chunk_size] = chunk_val            
    
    probabilities = np.zeros( (n, n_states) ) 
    for state in range(n_states): 
        for index in range(n):
            lam = emissions[state] * weights[index] * mutrates[index]
            # lam = emissions[state] * weights[index]
            # mut = np.random.poisson(mutrates[index], 1) 
            # lam = emissions[state] * weights[index] * mut[0]
            # lam = emissions[state] * weights[index] * mut[index]
...
def TrainModel(obs, mutrates, weights, hmm_parameters, epsilon = 1e-3, maxiterations = 100):

    # Get probability of data with initial parameters
    previous_loglikelihood = GetProbability(hmm_parameters, weights, obs, mutrates)
    # prob of obsering the SNPs in the window given the emissions parameters and starting state
...
def Simulate_values(p):
    ...
    # Use binom dist to randomly choose whether to move or stay in the current state
...
def Simulate_transition(n_states, matrix, current_state):
    ...
    if next_state == 1:
        ...
        if n_states == 2:
            ...
            # Can ignore this because we will only ever use 2 states
...
def Write_posterior_probs(obs, weights, mutrates, post_seq, post_path, viterbi_path, hmm_parameters, filename):
    post_seq = post_seq.T

    with open(filename, 'w') as out:
        state_names = '\t'.join(hmm_parameters.state_names)
        print('start', 'end', 'num_kmers', 'mutationrate', state_names, 'posterior_state', 'viterbi_state', sep = '\t', file = out)
        ...
        for (obs, m, posterior, post_state, viterbi_state) in zip(obs, mutrates, post_seq, post_path, viterbi_path):
            ...
            print(i * 2000, (i + 1) * 2000, obs, m, posterior_to_print, hmm_parameters.state_names[post_state], hmm_parameters.state_names[viterbi_state], sep = '\t', file = out)
            i += 1
