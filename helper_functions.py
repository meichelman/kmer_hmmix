...
def make_callability_from_bed(bedfile, window_size):
    ...
    # make nested dictionary with default 0 values
...
def load_obs_mut(obs_file, mutrates_file):
    contig1_name = ''
    # this is specific to the assembly - change for every new assembly!!!!
    
    # get contig length
    with open(obs_file, 'r') as infile:
        contig1_name = infile.readline().split('\t')[0]
        infile.seek(0)
        for line in infile:
            if line.startswith(contig1_name):
                # testing with kmer_hmmix/HG01891_h1/vidija/HG01891_h1_counts.bed contig 1
                if len(line.split('\t')) == 5:
                    contig, win_start, win_end, count, _ = line.split('\t')
                else:
                    contig, win_start, win_end, count = line.split('\t')
            
    window_size = 2000
    contig_len = int(win_end)
    windows = contig_len // window_size + 1

    # FOR NOW we are using weights of 1
    observations = np.zeros(windows, dtype=int)
    mutrates = np.zeros(windows, dtype=float)
    weights = np.ones(windows, dtype=float)
    ...
    with open(obs_file, 'r') as infile:
        for line in infile:
            if line.startswith(contig1_name):
                # testing with kmer_hmmix/HG01891_h1/vidija/HG01891_h1_counts.bed contig 1
                if len(line.split('\t')) == 5:
                    contig, win_start, win_end, kmer_count, _ = line.split('\t')
                else:
                    contig, win_start, win_end, kmer_count = line.split('\t')
                window_index = int(win_start) // window_size
                observations[window_index] = int(kmer_count)

    with open(mutrates_file, 'r') as infile:
        for line in infile:
            if line.startswith('contig'):
                continue
            elif line.startswith(contig1_name):
                # testing with kmer_hmmix/HG01891_h1/vidija/HG01891_h1_counts.bed contig 1
                contig, win_start, win_end, mutrate = line.strip().split('\t')
                idx_start = int(win_start) // window_size
                for window in range(int(win_end) // window_size):
                    window_index = idx_start + window
                    if window_index >= windows:
                        break
                    mutrates[window_index] = float(mutrate)
            
    # Make sure there are no places with obs > 0 and 0 in mutation rate or weight
    for index, (observation, w, m) in enumerate(zip(observations, weights, mutrates)):
    ...
    return observations, mutrates, weights
...
def find_runs(inarray):
    ...
    if n == 0: 
        ...
        y = ia[1:] != ia[:-1]
        # Boolean array where each element is True if the current element is different from the previous one
        i = np.append(np.where(y), n - 1)
        # Find indices where the value changes and append the last index
